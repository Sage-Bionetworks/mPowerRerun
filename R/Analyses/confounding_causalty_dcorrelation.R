...
#' for assessing demographics confounders
#' based on distance correlation
#' 
...
#######################################################
SCRIPT_NAME <- "confounding_causalty_dcorrelation.R"
...
MODEL_OUTPUT <- paste0("demo_confounders_distance_correlation_",
...
#' Pipeline for Confounding analysis based on distance correlation
#' 
#' This function will run confounding analysis based on distance correlation
#' It will run confounding analysis based on random forest
#' and ridge regression model and test performance based on subset of
#' data combination. It will use a combination of features from
#' demographics, demographics + sensor, sensor only. 
#' 
#' @param data activity features (with demographic information of age, 
#' education, gender, diagnosis)
...
#' @return A list of metrics (log(p-value), correlation) 
#' in matrix form for each activity 
confounding_dcorrelation_analysis_pipeline <- function(data, features, subsample, nRuns){
  set.seed(1234567)
  ...
  age.rf <- RunCondIndepTestsdCorRf(datM,
                                    nRuns,
                                    splitSeeds,
                                    labelName = "PD",
                                    ...
                                    negClassName = "FALSE",
                                    posClassName = "TRUE")
  gender.rf <- RunCondIndepTestsdCorRf(datM,
                                       nRuns,
                                       splitSeeds,
                                       labelName = "PD",
                                       featNames = featNamesC,
                                       ...
                                       negClassName = "FALSE",
                                       posClassName = "TRUE")
  education.rf <- RunCondIndepTestsdCorRf(datM,
                                          nRuns,
                                          splitSeeds,
                                          labelName = "PD",
                                          featNames = featNamesC,
                                          ...
                                          negClassName = "FALSE",
                                          posClassName = "TRUE")
  age.rr <- RunCondIndepTestsdCorRr(datM,
                                    nRuns,
                                    splitSeeds,
                                    labelName = "PD",
                                    featNames = featNamesC,
                                    ...
                                    negClassName = "FALSE",
                                    posClassName = "TRUE")
  gender.rr <- RunCondIndepTestsdCorRr(datM,
                                       nRuns,
                                       splitSeeds,
                                       labelName = "PD",
                                       featNames = featNamesC,
                                       ...
                                       negClassName = "FALSE",
                                       posClassName = "TRUE")
  education.rr <- RunCondIndepTestsdCorRr(datM,
                                          nRuns,
                                          splitSeeds,
                                          labelName = "PD",
                                          featNames = featNamesC,
...
main <- function(){
  ...
  results <- plyr::llply(.data = data.list, 
                         ...
                         .fun = function(activity){
                           data <- confounding_dcorrelation_analysis_pipeline(
                             data = activity$data, 
  ...
  synStore(f, activity = Activity(
    "Run Confounding Causalty Distance Correlation Analysis",
    executed = GIT_URL, 
